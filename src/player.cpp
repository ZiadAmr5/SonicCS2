...
{ // might include a case where both left and right are pressed , maybe a feature
    ...
    if (right)
        facing = 1; // remember which way we're facing (fireball direction)
    else if (left)
        facing = -1;
    ...
    if (right) {
        ...
        if (velocityX < 0) {
            ...
            if (velocityX > 0) {
                velocityX
                    = 0.5f; //this makes it so that when the player turns, they would start with a speed = 0.5
            }
        }
        //going right
    ...
    } else if (left) { //going right then pressing left
        if (velocityX > 0) {
    ...
    qDebug() << "velocityX:" << velocityX << "VelocityY" << velocityY << "Gravity:" << gravity
             << "MaxSpeed:" << currentMaxSpeed << "acceleration:" << acceleration << "Deceleration"
             << currentSkidding;
...
int Player::currentFrame() const
{
    if (!isOnGround)
        return 4; // jump
    if (abs(velocityX) > 5.0f)
        return 1 + (int(animTime) % 3); // walk cycle
    return 0;                           // idle
}
...
void Player::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    int srcY = 0, srcH = 16;
    if (power == PowerState::Big) {
        srcY = 16;
        srcH = 32;
    }
    if (power == PowerState::Fire) {
        srcY = 48;
        srcH = 32;
    }
...
void Player::stompEnemy(Enemy *e)
{
    if (e)
        e->stomped();
    addScore(200);
...
void Player::collectCoin(Coin *c)
{
    ...
    if (health < maxHealth)
        ...
        addScore(100); // full health -> points instead
    ...
    qDebug() << "coin! coins:" << coins << "health:" << health << "score:" << score
             << "lives:" << lives;
}
